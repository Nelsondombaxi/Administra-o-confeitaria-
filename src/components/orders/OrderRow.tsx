...
import { Eye, Clock, CheckCircle2, Flame, PackageCheck } from 'lucide-react';
...
export function OrderRow({ order, onViewDetails }: OrderRowProps) {
  ...
  const formattedPrice = new Intl.NumberFormat('pt-AO', {
    style: 'currency',
    currency: 'AOA',
  }).format(order.totalValue).replace('AOA', 'Kz');
    ...
    <div className="bg-white rounded-2xl border border-[#e6dec5] p-5 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4 hover:border-[#c5a059]/50 transition-all">
      <div className="space-y-1">
        <div className="flex items-center gap-3">
          <span className="text-sm font-black text-[#2b1810] font-mono">{order.id}</span>
        ...
        <p className="text-xs text-[#5c3524]">{order.productName} • <span className="text-[#8c5338]">{order.createdAt}</span></p>
      </div>
